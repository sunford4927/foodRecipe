...
const AddBundle = () => {
    ...
    const [inputGroups, setInputGroups] = useState([0]); // 총 2개의 AddInput을 위해 초기화
    ...
    const removeInputGroup = () => {
        if (inputGroups.length > 1) { // 맨 처음 AddInput 포함 최소 3개는 유지
        ...
        <div className="bundleContainer"> {/* 전체 컨테이너 div */}
            <div className="inputRow"> {/* 재료 입력 필드와 AddInput을 포함하는 행 */}
                <Form.Control 
                    ...
                    onChange={handleChangeB} // 값이 변경될 때 handleChange 함수 호출
                />
            ...
            <div className='InputAllBtn mx-auto'>
                ...
                {inputGroups.length > 1 && ( // inputGroups 길이가 2보다 클 때만 삭제 버튼 보이기
